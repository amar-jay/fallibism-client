...
const sizeClassnames: Record<Exclude<ButtonProps["size"], null | undefined>, string> = {
  lg: "py-2 px-6 text-sm rounded-lg",
  sm: "px-2 py-1 text-sm rounded-md",
  default: "px-1 text-sm rounded-sm",
};

const colorClassnames: Record<Exclude<ButtonProps["variant"], null | undefined | "link">, string> = {
  default:
...
export const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  return (
    <button
      ...
      className={cn("flex outline-none focus:ring-4 ",
      "focus:ring-"+ variant,
        sizeClassnames[size!],
...
export const SidebarGroup = ({ title, items, className }: {
  title: string;
