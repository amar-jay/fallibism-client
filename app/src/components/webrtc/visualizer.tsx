...
const SoundColumn = (props: SoundColumnsProps) => {
  ...
  return (
    <div
	style={{ display: 'flex', gap: props.config.barGap, flexDirection: "column" }}
	>
      {items.map((_, index) => {
        ...
        return (
          <div
          key={'sound-column-item-' + index}
          className={cn('flex flex-row items-center',
          "gap-" + Number(props.config.barGap) * 4,
          'sound-column', 
          isOff ? 'box-sha' : 'sound-column-on',
          'w-' + props.config.columnWidth,
          "bg-[" + backgroundColor + "]",
...
const ThinkingView = ({ config }: { config: ConfigType }) => {
  ...
  return (
    <div
    className={cn('flex flex-row items-center opacity-50 ease-out transition-opacity',
    "gap-" + Number(config.barGap) * 4,
    )}
      // flexDirection="row"
      // gap={config.barGap}
      // alignItems="center"
    ...
    >
      {config.barCounts.map((_, idx) => {
        ...
        return (
          <div
          style={{ width: config.columnWidth, height: config.barHeight,
...
const HelperView = () => {
  return (
    <div
    className='absolute py-4 text-sm text-white opacity-80 top-full text-center leading-normal'
      // position="absolute"
      // py="1rem"
      // fontSize="0.75rem"
